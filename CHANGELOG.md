...
- fix: Read the `enabled` option through the schema, so `enabled: no` disables the filter. Comparing the document text left every spelling but `false` rewriting links, and reported nothing. (#25)
...
- build: Update the vendored Lua modules to 2.5.0, which adds the accessor that reads what the schema resolves an option to. The schema validator moves to 3.6.3, whose vendored file is unchanged. (#25)
